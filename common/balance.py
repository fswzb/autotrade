...
def stop_loss_by_price():
    ...
    for symbol in symbolAccount.avail_secpos.keys():
        ...
        if max(close_prices) >= (1 + threshold_profit_open
                                 ) * close_price_begin:  #止盈策略启动
            max_price = max(close_prices)
            if realtime_price <= (1 - threshold_profit_pretect) * max_price:
                #符合止盈策略,卖出
                reason = '符合止盈策略:相比最高价,下跌了{:.2}%'.format(
                    (max_price - realtime_price) / max_price * 100)
                symbolAccount.order(
                    symbol, symbolAccount.avail_secpos[symbol], realtime_price,
                    time.strftime("%Y-%m-%d", time.localtime(time.time())))
                judgements.append((symbol, -1, reason))

        elif realtime_price <= (1 - threshold_loss_pretect
                                ) * close_price_begin:
            #符合止损策略,卖出
            down = (
                close_price_begin - realtime_price) / close_price_begin * 100
...
class SymbolAccount():
    ...
    def __init__(self, account=100000):
        ...
        self.avail_secpos = {
        }  # 字典，键为证券代码，值为持有的证券数量, {'000001': 100, '600000': 100}
        self.order_history = {
        }  #dict, {symbol:000001, [(order_time:2016-07-11, , order_amount:100, order_price)]} , order_amount为正时买入,为负时卖出
        self.position_history = [
        ]  # 持仓历史[2016-07-11, cash, {'600000':100, '600031':300}]
    ...
    def init_position_from_db(self):
        ...
        :return:
        """
        sql = "SELECT * FROM trade_order order by trade_time asc"
        ...
        for code in df['code'].drop_duplicates():
            ...
            for _, row in df_code.iterrows():
                self.order_history[code].append([str(row['trade_time']),
                                                 row['trade_count'],
                                                 row['trade_price']])
    ...
    def order(self, smybol, amount, price, trade_date):
        ...
        if amount > 0:  # 买入
            if amount * price < self.cash:
                ...
                self.order_history[smybol].append(
                    [str(trade_date)[:10], amount, price])
            else:
                return False
        ...
        self.position_history.append(
            [trade_date, self.cash, copy.deepcopy(self.avail_secpos)])
    ...
    def get_current_account(self, current_date):
        ...
        for symbol in self.avail_secpos.keys():
            ...
            if close_hfq == -1:
                msg = "{},{},get code close_price error!".format(symbol,
                                                                 current_date)
                # raise Exception(msg)
                return 0
    ...
    def get_sorted_symbols_by_return_rate(self, trade_date):
        ...
        for smybol in self.order_history.keys():
            ...
            smybol_rate_list.append((smybol,
                                     (now_price - aver_price) / aver_price))

        smybol_rate_list = sorted(
            smybol_rate_list, key=itemgetter(1), reverse=True)
    ...
    def calcute_maximun_drawdown(self):
        ...
        if len(jingzhi_list) >= 2:
            jingzhi_list_sorted = sorted(
                jingzhi_list, key=lambda x: x[1], reverse=True)
            max_vale = jingzhi_list_sorted[0][1]
            max_value_date = jingzhi_list_sorted[0][0]
            jingzhi_list_after = [jingzhi for jingzhi in jingzhi_list
                                  if jingzhi[0] > max_value_date]
    ...
    def get_jingzhi_daliy(self):
        ...
        # 持仓起止日期范围
        rng = pd.date_range(
            self.position_history[0][0], self.position_history[-1][0],
            freq='D')
        jingzhi_list = []
    ...
    def get_position_by_trade_date(self, trade_date):
        ...
        for i in range(1, len(self.position_history)):
            if str(trade_date.to_datetime()) >= str(self.position_history[
                    i - 1][0]) and str(trade_date.to_datetime()) < str(
