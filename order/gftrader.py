...
from order.webtrader import WebTrader
from configs import *
...
class GFTrader(WebTrader):
    def __init__(self):
        ...
        self.cookie = None
        self.account_config = guangfa_config
        self.s = None
        self.exchange_stock_account = dict()
        self.sessionid = ''
        self.holdername = list()
        self.config = gf_config
    ...
    def create_basic_params(self):
        basic_params = dict(dse_sessionId=self.sessionid)
        return basic_params
    ...
    def __get_trade_need_info(self, stock_code):
        """获取股票对应的证券市场和帐号"""
        # 获取股票对应的证券市场
        exchange_type = self.holdername[SH]['exchange_type'] if order.get_stock_type(stock_code) == 'sh' \
        ...
        return dict(exchange_type=exchange_type, stock_account=stock_account)
    ...
    def cnjj_apply(self, stock_code, amount):
        ...
        :param amount: 申购金额
        """
        params = dict(self.config['cnjj_apply'], entrust_amount=amount)
    ...
    def cnjj_redemption(self, stock_code, amount=0):
        ...
        :param amount: 赎回份额
        """
        params = dict(self.config['cnjj_redeem'], entrust_amount=amount)
    ...
    def nxbQueryPrice(self, fund_code):
        """牛熊宝查询
        """
        params = dict(self.config['nxbQueryPrice'], fund_code=fund_code)
    ...
    def nxbentrustcancel(self, entrust_no):
        ...
        """
        params = dict(self.config['nxbentrustcancel'], entrust_no=entrust_no)
        return self.do(params)
    ...
    def queryOfStkCodes(self):
        ...
        """
        params = dict(
            self.config['queryOfStkCodes'], prodta_no="98", business_type="2")
        return self.do(params)
    ...
    def getStockQuotation(self, stockcode):
        ...
        response_data = response_data[response_data.find('hq') + 3:
                                      response_data.find('hqtype') - 1]
