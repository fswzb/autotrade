...
class HTTrader(WebTrader):
    def __init__(self, remove_zero=True):
        ...
        self.account_config = huatai_config
        self.s = None
        self.remove_zero = remove_zero
        self.config = ht_config
    ...
    def __set_ip_and_mac(self):
        self.__ip = self._get_ip()
        self.__mac = self._randommac()
    ...
    def __get_user_name(self):
        ...
        use_index_start = 1
        return raw_name[use_index_start:] if raw_name.startswith(
            '08') and self.remove_zero is True else raw_name
    ...
    def __set_trade_need_info(self, json_data):
        ...
        for account_info in json_data['item']:
            if account_info['stock_account'].startswith('A') or account_info[
                    'stock_account'].startswith('B'):
    ...
    def cancel_entrust(self, entrust_no):
        ...
        cancel_params = dict(
            self.config['cancel_entrust'], entrust_no=entrust_no)
    ...
    def __get_trade_need_info(self, stock_code):
        """获取股票对应的证券市场和帐号"""
        # 获取股票对应的证券市场
        exchange_type = self.__sh_exchange_type if order.get_stock_type(stock_code) == 'sh' \
    ...
    def request(self, params):
        ...
        r = self.s.get('{prefix}/?{b64params}'.format(
            prefix=self.trade_prefix, b64params=b64params),
                       headers=headers)
        return r.content
    ...
    def format_response_data(self, data):
        ...
        if response_data['cssweb_code'] == 'error' or response_data[
                'item'] is None:
