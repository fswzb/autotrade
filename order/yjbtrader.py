...
class YJBTrader(WebTrader):
    def __init__(self):
        ...
        self.account_config = yongjinbao_config
        self.config = yjb_config
        self.config.update(self.global_config)
        self.trade_prefix = self.config['prefix']
        self.s = requests.session()
        self.s.mount('https://', order.Ssl3HttpAdapter())
    ...
    def handle_recognize_code(self):
        ...
        verify_code = order.recognize_verify_code(image_path, 'yjb')
    ...
    def get_ipo_limit(self, stock_code):
        ...
        if 'error_no' in data.keys() and data['error_no'] != "0":
            ...
            return None
        return dict(
            high_amount=float(data['high_amount']),
    ...
    def __get_trade_need_info(self, stock_code):
        ...
        if exchange_type not in self.exchange_stock_account:
            ...
            response_data = self.do(
                dict(
                    self.config['account4stock'],
                    exchange_type=exchange_type,
                    stock_code=stock_code))[stock_account_index]
            self.exchange_stock_account[exchange_type] = response_data[
                'stock_account']
        return dict(
            exchange_type=exchange_type,
    ...
    def fix_error_data(self, data):
        error_index = 0
        return data[error_index] if type(data) == list and data[
            error_index].get('error_no') is not None else data
